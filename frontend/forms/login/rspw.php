...
if (isset($_POST['reset'])) {
    ...
    if (count($error) === 0) {
        ...
        $pd = trim($pd);
        $sql = "UPDATE taikhoan SET password='$pd' WHERE email='$email' ";
        if (mysqli_query($conn, $sql)) {
            $success = 'success';
            session_destroy();
            unset($_SESSION['email']);
            echo '<script> 
            alert("Đổi mật khẩu thành công! Quay lại trang đăng nhập để đăng nhập lại!");
            location.href = "/nln_test/frontend/forms/login/login.php?reset=' . $success . '" </script>';
            exit();
        } else {
            $error['db'] = 'Không đổi được mật khẩu. Hãy liên hệ với admin thông qua <a href="/nln_test/frontend/forms/QA/lienhe.php">link này</a>';
        }
    }
}
?>
