...
$query = ' 	SELECT l.id, l.name, l.phone, l.email, l.content, l.is_checked, l.up_time
            FROM lien_he l
';

if ($_POST['query'] != '') {
    $query .= 'WHERE l.name LIKE "%' . str_replace(' ', '%', $_POST['query']) . '%" ';
}
...
$output = '
    ...
    <thead class="thead-dark">
        <tr>
            ...
            <th class="text-center">Người góp ý</th>
            <th class="text-center">Nội dung góp ý</th>
            <th class="text-center">Thời điểm góp ý</th>
...
if ($total_row > 0) {
    while ($row = mysqli_fetch_assoc($rs)) {
        if ($row['is_checked'] == 1) {
            ...
            $verify = '<button class="btn btn-secondary btn-check" data-contact-id="'.$row["id"].'">Chưa duyệt</button>';
        }
        $output .= '
            <tr>
                <td>
                ...
                <td class="text-center">' . date('H:i:s d/m/Y',strtotime($row['up_time'])) . '</td>
                <td class="text-center">' . $verify . '</td>
                <td class="text-center">
                    <button type="button" class="btn btn-danger btn-del" data-contact-id="' . $row['id'] . '">Xóa</button>
                </td>
             </tr>
        ';
    }
...
';
    ...
    $output .= '
    <tr>
        <td colspan="6" align="center">Không tìm thấy góp ý</td>
    </tr>
    ';
}
