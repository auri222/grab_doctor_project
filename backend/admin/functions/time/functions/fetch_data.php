...
$query = ' 	SELECT k.id, k.name as GIO, b.name as BUOI
            FROM khung_gio k
            JOIN buoi b ON k.id_buoi = b.id
';
    ...
    while ($row = mysqli_fetch_assoc($rs)) {
        $output .= '
            <tr>
                ...
                <td class="text-center">' . $row['BUOI'] . '</td>
                <td class="text-center">
                    <a href="/nln_test/backend/admin/functions/time/functions/edit.php?time_id='.$row['id'].'" class="btn btn-warning">Sửa</a>
                    <button type="button" data-time-id='.$row['id'].'" class="btn btn-danger btn-delete">Xóa</button>
                </td>
            </tr>
        ';
    }
